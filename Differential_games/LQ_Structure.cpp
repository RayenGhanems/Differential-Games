...
void readCSV(const string &filename, Dg::State &S, const Dg::Constants &C) {
    ...
    while (getline(file, line) && i < t) {
        ...
        S.Ud.push_back(S.I * stod(parts[2]) + S.D * stod(parts[1]) + S.M * S.g * S.l * cos(stod(parts[0])));
        i++;
    }
...
void DG_fxn(Dg::Planning &P, const Dg::Constants &C) {
    ...
    for (int j = C.np-1; j > 0; j-=C.integration_step) {  
        ...
        Art_i.noalias() = P.A.block(0,j*C.n,C.n,C.n);
        Aht_i.noalias() = P.A.block(0,j*C.n,C.n,C.n) - Brt_i * P.Pr;

        crt_i.noalias() = P.C.block(0,j,C.n,C.m);
        ...
        // Update equations
        P.Pr.noalias() += C.integration_time_step * (Frt_i + Frt_i.transpose() + P.Qr - P.Pr * Brt_i * P.Pr );
        P.ar.noalias() += C.integration_time_step * ((Art_i - Brt_i * P.Pr).transpose() * P.ar + P.Pr * crt_i );
