...
void writeUd(const vector<float> Ud);
...
double Estimation_fxn(const vector<double>& x, vector<double>& grad, void* data) {
    ...
    for (int i = 0; i < DG.C.ne; i++) {
        ...
        for (int j = DG.C.np - 1; j > 0; j -= DG.C.integration_step) {  
            ...
            Art_i.noalias() = DG.E.A.block(0, (j + i) * DG.C.n,DG.C.n,DG.C.n);
            Aht_i.noalias() = DG.E.A.block(0, (j + i) * DG.C.n,DG.C.n,DG.C.n) - Brt_i * DG.E.Pr;

            crt_i.noalias() = DG.E.C.block(0, (j + i),DG.C.n,DG.C.m);
            ...
            // Update equations
            DG.E.Pr.noalias() += DG.C.integration_time_step * (Frt_i + Frt_i.transpose() + DG.E.Qr - DG.E.Pr * Brt_i * DG.E.Pr);
            DG.E.ar.noalias() += DG.C.integration_time_step * ((Art_i - Brt_i * DG.E.Pr).transpose() * DG.E.ar + DG.E.Pr * crt_i);
...
void writeToCSV(const vector<VectorXd>& Ur_arr,const vector<VectorXd>& Uh_arr) {
    ofstream file("/home/ray/Desktop/a./New Folder/outputs_LQ/Ur_Uh_LQ.csv");
    if (file.is_open()) {
...
void writeUd(const vector<float> Ud) {
    ofstream file("/home/ray/Desktop/a./New Folder/outputs_LQ/Ud_Lq.csv");
    if (file.is_open()) {
...
void writeX(const vector<vector<float>>& data) {
    std::ofstream file("/home/ray/Desktop/a./New Folder/outputs_LQ/X.csv");
